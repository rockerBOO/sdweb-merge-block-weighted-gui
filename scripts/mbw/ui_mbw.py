...
def on_ui_tabs():
        ...
        with gr.Row():
            ...
            with gr.Column():
                dd_preset_weight = gr.Dropdown(
                    label="Preset Weights", choices=presetWeights.get_preset_name_list()
                )
    ...
    def onclick_btn_do_merge_block_weighted(
        ...
        if not chk_allow_overwrite:
            if os.path.exists(_output):
                _err_msg = (
                    "ERROR: output_file already exists. overwrite not allowed. abort."
                )
                print(_err_msg)
    ...
    btn_reload_checkpoint_mbw.click(
        fn=on_btn_reload_checkpoint_mbw, inputs=[], outputs=[model_A, model_B]
    )
